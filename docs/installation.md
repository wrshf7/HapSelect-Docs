...
### Important Notes 
- The locations described are realtive to the root folder in the R package directory. Please download and extract the `.zip` file and set your current directory to the root folder. This can be accomplished with the `cd` command from `bash` on a Linux/Unix or MacOS command line, for exmaple. Example command: `cd /User/Will/HapSelect`.
